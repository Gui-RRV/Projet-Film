   ...
   $(document).ready(function(){
    ...
    <body>
      <div class="site-container">
        <header>   
<?php
if (isset($_SESSION['user']) && isset($_SESSION['id'])) { //Si l'utilisateur est connecté alors on affiche son pseudo à la place de "mon compte" et on lui permet d'accéder à son profil ainsi que la possiblité de se déconnecter  

          echo '<div id="disparu">';

      echo '<div id="moncompte" ><a href="#" target=""> <p>'  .$_SESSION['user']. '</p> </a></div>';
 
            echo'<div id="test" ><a href="profil.php" target=""> <p> Mon profil </p> </a></div>';

            echo'<div id="test2" ><a href="deco.php" target=""> <p> Déconnexion </p> </a></div>';
      ...
      echo '<div id="moncompte" ><a href="#" target=""> <p> mon compte </p> </a></div>';
 
            echo'<div id="test" ><a href="inscription.php" target=""> <p> inscription </p> </a></div>';

           echo '<div id="test2" ><a href="connexion.php" target=""> <p> connexion </p> </a></div>';
...
$sth = $dbh->prepare("SELECT *, note/votes as notefi FROM films WHERE titre LIKE ? ");

$sth->execute([$searchFI]);
$count=$sth->rowCount();
$valeurs = $sth->fetch();
...
if($count == 0){
	...
	$sth->closeCursor();
	$sth = $dbh->prepare("SELECT *, note/votes as notefi FROM films WHERE realisateur LIKE ? ");
	...
	$sth->closeCursor();
	$sth = $dbh->prepare("SELECT *, note/votes as notefi FROM films WHERE genre LIKE ? ");
	...
	$sth->closeCursor();
	$sth = $dbh->prepare("SELECT *, note/votes as notefi FROM films WHERE pays LIKE ? ");
	...
	$sth->closeCursor();
	$sth = $dbh->prepare("SELECT *, note/votes as notefi FROM films WHERE synopsis LIKE ? ");
	...
	$sth->closeCursor();
	$sth = $dbh->prepare("SELECT *, note/votes as notefi FROM films WHERE date LIKE ? ");
	...
	$sth->closeCursor();
	$sth = $dbh->prepare("SELECT *, note/votes as notefi FROM films WHERE acteurs LIKE ? ");
	...
	$valeurs = $sth->fetch();
	 ...
	 echo ('<script> location.replace("erreur.php"); </script>');
